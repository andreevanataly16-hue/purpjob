...
class Settings(BaseSettings):
    ...
    # Как открывается регистрация.
    #
    # "invite" - закрытый пилот: нужен код приглашения. Это рабочий режим и
    # значение по умолчанию, потому что почта не подтверждается: при открытой
    # регистрации любой мог бы завести профиль на чужой адрес, и продукт
    # выглядел бы так, будто адрес проверен, хотя это не так.
    #
    # "open" - регистрация без кода. Публично так включать нельзя, пока нет
    # подтверждения почты; режим оставлен для локальной разработки и тестов,
    # и README говорит об этом прямо.
    registration_mode: str = "invite"

    # Код приглашения закрытого пилота. Задаётся в .env, в репозиторий не
    # попадает. Пустой код в режиме "invite" означает "регистрация закрыта":
    # свежая поставка не должна случайно оказаться открытой для всех.
