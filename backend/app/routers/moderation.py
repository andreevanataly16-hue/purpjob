...
ДОСТУП. Очередь открыта только роли модератора, и проверяется это на сервере:
страж стоит на роутере целиком, роль читается из записи пользователя, а не из
запроса. Кандидат и рекрутер сюда не попадают - ни через экран, ни прямым
обращением к API. Каждое обращение записывается в журнал.

Чего пока нет: разграничения по компаниям, срока действия роли и управления
ролями из интерфейса - роль выдаёт администратор скриптом. Для пилота этого
достаточно, для многопользовательской среды - ещё нет.
"""
