...
def test_first_stage_hides_exactly_name_and_photo(signed_client, recruiter_client):
    """FR1.1: закрыто два поля, всё остальное открыто целиком."""
    payload = detail(recruiter_client)
...
def test_hidden_identity_reads_as_intentional_not_broken(signed_client, recruiter_client):
...
def test_anonymous_label_is_stable_across_views(signed_client, recruiter_client):
    """§7: иначе рекрутер не узнает того, кого смотрел вчера."""
    first = detail(recruiter_client)["display_name"]
    recruiter_client.get(SEARCH, params={"vacancy_id": VACANCY})
    second = detail(recruiter_client)["display_name"]
...
def test_search_results_start_anonymous(signed_client, recruiter_client):
    payload = recruiter_client.get(SEARCH, params={"vacancy_id": VACANCY}).json()
...
def test_comparison_also_starts_anonymous(signed_client, recruiter_client):
...
def test_identity_opens_only_after_a_deliberate_action(signed_client, recruiter_client):
    """FR2.1: не по таймеру, не при открытии карточки — отдельным действием."""
    for _ in range(3):
        assert detail(recruiter_client)["identity_revealed"] is False

    after = advance(recruiter_client)
    assert after["identity_revealed"] is True
    assert after["display_name"] == "Мария К."


def test_advancing_does_not_change_anything_else(signed_client, recruiter_client):
...
def test_stage_is_per_recruiter_not_global(signed_client, client, credentials, recruiter_client):
    """FR2.2: этап одного рекрутера ничего не значит для другого."""
    advance(recruiter_client)
    assert detail(recruiter_client)["identity_revealed"] is True
...
def test_advancing_is_idempotent(signed_client, recruiter_client):
    first = advance(recruiter_client)
    second = advance(recruiter_client)
...
def test_advancing_fires_the_recruiter_interest_trigger(signed_client, recruiter_client):
    ...
    advance(recruiter_client, candidate_id)
...
def test_seed_candidate_produces_no_trigger(signed_client, recruiter_client):
    """Синтетическому профилю некому возвращаться — повода быть не должно."""
    advance(recruiter_client, FIRST)
...
def test_contacts_need_both_sides(signed_client, recruiter_client):
    """FR2.4/FR3.3: одного действия рекрутера здесь недостаточно."""
    go_public(signed_client)
    candidate_id = live_id()
    advance(recruiter_client, candidate_id)

    asked = recruiter_client.post(
...
def test_contacts_cannot_be_requested_before_identity(signed_client, recruiter_client):
...
def test_candidate_sees_who_reached_which_stage(signed_client, recruiter_client):
    """FR3.1: прозрачность, а не право вето."""
    go_public(signed_client)
    candidate_id = live_id()
    detail(recruiter_client, candidate_id)
...
def test_opt_out_skips_straight_to_identity(signed_client, recruiter_client):
    go_public(signed_client)
    signed_client.put(f"{REVEAL}/settings", json={"allow_immediate_identity_reveal": True})

    payload = detail(recruiter_client, live_id())
    assert payload["identity_revealed"] is True
    assert payload["current_stage"] == reveal.STAGE_2


def test_immediate_identity_does_not_imply_contact_consent(signed_client, recruiter_client):
...
def test_hidden_profile_is_never_staged_at_all(signed_client, recruiter_client):
