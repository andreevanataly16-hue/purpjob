    ...
    <div class="prof-head">
      <div>
        ...
        <p class="sub" style="margin-bottom:0">
          Не пересказ резюме, а измеримые характеристики: подтверждённый опыт,
          наложенный на эталон роли.
        </p>
      </div>
    </div>
...
function renderBody(prof) {
  ...
  if (!prof.snapshots.length) {
    box.innerHTML = `
      ${rolesRow(prof)}
      <div class="empty">
        Выберите целевой уровень — и увидите, насколько подтверждённый опыт закрывает эталон роли.
  ...
  box.innerHTML = `
    ...
    <p class="disclaimer">
      Веса эталона и перевод статуса в баллы — рабочая гипотеза первого сегмента, а не
      проверенная формула: числа будут меняться после калибровки.
    </p>
  `
}
