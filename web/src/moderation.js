/* Очередь модератора (модуль 7).
   ...
   Очередь открыта только роли модератора, и проверяется это на сервере — экран
   ничего не решает сам. Что уже закрыто доступом, а чего в нём ещё нет, стоит
   прямо в очереди, а не только в README: границу должно быть видно оттуда, где
   ей пользуются.
...
export function renderModeration(state) {
  ...
  box.innerHTML = `
    <div class="mod-warning access-note">${escape(queue.access_note_ru)}</div>
